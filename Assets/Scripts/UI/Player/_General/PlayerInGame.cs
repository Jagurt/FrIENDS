...
{
    static CustomNetManager CustomNetworkManager;
    ...
    //      Stats      //
    [SyncVar] [SerializeField] private int level = 1;         // Level of a player
    public int Level
    {
        get => level;
        set
        {
            if (gameManager.turnActiveffects.Find(item => item.CompareTag("PreventLevelUpEffect")))
            {
                InfoPanel.Alert("Level Up Prevention In Effect!\n No levels gained!");
                return;
            }
            level = value;
    ...
    [SerializeField] List<GameObject> activeEffects = new List<GameObject>();
    ...
    private void ClientInitialize()
    {
        ...
        if (hasAuthority)
        {
            ...
            if (PlayerCanvas.singleton)
                playerCanvas = PlayerCanvas.singleton.gameObject;
            ...
            {
                playerStatsUI = playerCanvas.transform.Find("PlayerStats").gameObject;
                StartCoroutine(playerStatsUI.GetComponent<PlayerStats>().Initialize(this));
    ...
    private void ServerSendCard( Deck deck, bool worksOnDrawingPlayer = false, bool choice = false ) // Drawing card from deck and sending to Clients
    {
        ...
        switch (deck)
        {
            case Deck.Doors:
                ...
                drawCardZone = GameManager.singleton.ServerDecksManager.DoorsDeck.GetComponent<DrawCardZone>();
                last = GameManager.singleton.ServerDecksManager.Doors.Count - 1; // Number of cards in deck - 1
                break;
            case Deck.Treasures:
                drawCardZone = GameManager.singleton.ServerDecksManager.TreasuresDeck.GetComponent<DrawCardZone>();
                last = GameManager.singleton.ServerDecksManager.Treasures.Count - 1;
                break;
    ...
    private void ClientReceiveCard( GameObject card, bool worksOnDrawingPlayer = false, bool choice = false )
    {
        ...
        if (hasAuthority) // if is local player - takes drawn card and puts it in hand
        {
            ...
            {
                if (!ChoicePanel.singleton.gameObject.activeInHierarchy)
    ...
    void CmdUseCard( NetworkInstanceId cardNetId, NetworkInstanceId targetNetId )
    {
        ...
        StartCoroutine(card.ServerQueueCard(targetNetId, this.netId));
    }
    ...
    {
        gameManager.cardsUsageQueue[0].GetComponent<Card>().ServerConfirmCardUsage(confirm, gameObject);
    }
    ...
    {
        gameManager.cardsUsageQueue[0].GetComponent<Card>().ServerInterruptCardUsage();
    }

    internal void DiscardCard( List<GameObject> cardsToDiscard )
    {
        foreach (var cardGO in cardsToDiscard)
        {
            cardGO.GetComponent<Draggable>().ClientStopLeanMovement(true);
            CmdDiscardCard(cardGO.GetComponent<Card>().netId);
        }
    }
    ...
    [Server]
    static IEnumerator ServerDiscardCards( List<GameObject> cardsToDiscard )
    {
        foreach (var card in cardsToDiscard)
        {
            ...
            localPlayerInGame.RpcDiscardCard(card.GetComponent<Card>().netId);
    ...
    /// <summary> Moving card to its coresponding discarded deck and updating UI accordingly. </summary>
    ...
    internal void RpcDiscardCard( NetworkInstanceId cardNetId )
    {
        ...
        switch (cardScript.cardValues.deck)
        {
            case Deck.Doors:
                ...
                GameManager.singleton.ServerDecksManager.DiscardedDoorsDeck.GetComponent<DrawCardZone>().ReceiveCard(card.transform);
                break;
            case Deck.Treasures:
                cardScript.deck = Deck.DiscardedTreasures;
                GameManager.singleton.ServerDecksManager.DiscardedTreasuresDeck.GetComponent<DrawCardZone>().ReceiveCard(card.transform);
                break;
            case Deck.HelpingHand:
                break;
            case Deck.Spells:
    ...
    [Server]
    internal static IEnumerator ClearTable()
    {
        List<GameObject> tableGOs = new List<GameObject>();

        foreach (var card in TableDropZone.singleton.transform.GetComponentsInChildren<Card>())
        {
            tableGOs.Add(card.gameObject);
        }
        return ServerDiscardCards(tableGOs); // Discarding cards from table
    }
    ...
    void RpcEndFight()
    {
        ...
        GameManager.singleton.helpingPlayerEffects.Clear();
        GameManager.singleton.fightingPlayerEffects.Clear();
        GameManager.singleton.fightingMonsters.Clear();
    ...
    internal void RequestHelp()
    {
        if (gameManager.offeringHelpPlayers.Count > 0)
        {
            ...
            ChoicePanel.ReceivePlayersToChoose(gameManager.offeringHelpPlayers.ToArray());
        }
    }
    ...
    void RpcOfferHelp()
    {
        ...
        if (gameManager.offeringHelpPlayers.Contains(this))
            return;
        // Adding player who offered help to their list.
        gameManager.offeringHelpPlayers.Add(this);

        // Updating RequestHelp button text for fighting player
        if (localPlayerInGame.netId == gameManager.fightingPlayerNetId)
    ...
    {
        if (gameManager.offeringHelpPlayers.Contains(this))
            gameManager.offeringHelpPlayers.Remove(this);
    ...
    IEnumerator ServerChoosePlayerToHelp( NetworkInstanceId helpingPlayerNetId )
    {
        ...
        gameManager.helpingPlayerNetId = helpingPlayerNetId;
        gameManager.fightingPlayersLevel += ClientScene.FindLocalObject(helpingPlayerNetId).GetComponent<PlayerInGame>().level;
    ...
    {
        HelpButton.DeactivateButton();
        gameManager.offeringHelpPlayers.Clear();      // Clearing players which offered help 
